...
extension UIApplication {
    /// This is an extension method offered by the Clip SDK that allows you to present a View Controller modally on top of the rest of your app from a non-UI context such as an AppDelegate method for receiving a tapped push notification, opening a deep link, etc. You may use this to present the [ClipViewController](x-source-tag://ClipViewController).
    ///
    ...
    public func present(
        ...
        if viewControllerToPresent.parent != nil {
            clip_log(.default, "Failed to present viewControllerToPresent - already presented in an unsupported container")
            completion?()
            return
        }
        ...
        guard let keyWindow = self.windows.first(where: \.isKeyWindow), let rootViewController = keyWindow.rootViewController else {
            clip_log(.error, "Failed to present viewControllerToPresent - rootViewController not found")
            completion?()
            return
        }
        ...
        guard let visibleViewController = findVisibleViewController?(rootViewController) else {
            clip_log(.error, "Failed to present `viewControllerToPresent` - visible view controller not found")
            completion?()
            return
        }
