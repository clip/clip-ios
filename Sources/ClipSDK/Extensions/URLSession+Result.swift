...
extension URLSession {
    ...
    func dataTask(with request: URLRequest, completionHandler: @escaping (Result<Data, NetworkError>) -> Void) -> URLSessionDataTask {
        dataTask(with: request) { data, response, error in
            // the usages of removeCachedResponse() below, while a bit hacky, are present to prevent the use of .returnCacheDataElseLoad in fetchDocumentData from repeatedly returning a cached error response.
