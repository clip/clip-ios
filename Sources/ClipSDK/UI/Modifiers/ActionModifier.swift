...
struct ActionModifier: ViewModifier {
    ...
    @Environment(\.navBarAppearance) private var navBarAppearance
    ...
    func body(content: Content) -> some View {
        if let action = layer.action, let document = document {
            Button {
                action.handle(document: document, navBarAppearance: navBarAppearance, show: showAction, present: presentAction, dismiss: dismiss)
            } label: {
                content
            }
