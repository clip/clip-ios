...
extension UINavigationItem {
    func configure(navBar: NavBar, stringTable: StringTable, traits: UITraitCollection, buttonHandler: @escaping (NavBarButton) -> Void) {
        ...
        leftBarButtonItems = navBar.children
            ...
            .map { navBarButton in
                UIBarButtonItem(navBarButton: navBarButton, stringTable: stringTable) {
    ...
    func configureLargeAppearance(navBar: NavBar, traits: UITraitCollection) {
        ...
        largeAppearance.configureBackground(background: navBar.appearance.background, traits: traits)
        largeAppearance.configureButtonColor(appearance: navBar.appearance, traits: traits)
        largeAppearance.configureTitleColor(appearance: navBar.alternateAppearance ?? navBar.appearance, traits: traits)
        largeAppearance.configureLargeTitleColor(appearance: navBar.appearance, traits: traits)
        scrollEdgeAppearance = largeAppearance
        
        if let alternateAppearance = navBar.alternateAppearance {
...
private extension UIBarButtonItem {
    private struct AssociatedObject {
        static var key = "io.clip.UIBarButtonItem.onTap"
    }
    ...
    convenience init(navBarButton: NavBarButton, stringTable: StringTable, onTap: @escaping () -> Void) {
        ...
        case .custom:
            if let icon = navBarButton.icon {
                ...
                self.init(
                    title: navBarButton.title.map { stringTable.resolve(key: $0) },
                    style: .plain,
                    target: nil,
                    action: nil
                )
