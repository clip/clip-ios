...
final class AssetsDownloader {
    ...
    func enqueue(url: URL, priority: Priority, completion: @escaping (Result<Data, Swift.Error>) -> Void) {
        if let task = tasks.first(where: { $0.task.originalRequest?.url == url && $0.state != .completed }) {
            if task.priority < priority {
                // re-prioritize
                clip_log(.debug, "Re-priotize %@ -> %@ request: %@", task.priority.rawValue, priority.rawValue, url.absoluteString)
                ...
                clip_log(.debug, "Enqueue %@ request: %@\t[skip duplicate]", priority.rawValue, url.absoluteString)
                task.subscribe(completion)
                return
            }
        } else {
            clip_log(.debug, "Enqueue %@ request: %@", priority.rawValue, url.absoluteString)
        }

        let taskIdentifier = UUID().uuidString
        let request = URLRequest.clip(url: url)
        ...
        let urlTask = session.dataTask(with: request) { result in
            ...
            self.tasks.filter({ $0.id == taskIdentifier }).forEach {
                clip_log(.debug, "Did download %@", $0.url.absoluteString)
                $0.notify(Result {
                    try result.get()
                })
            }
            ...
            clip_log(.debug, "%@ download tasks left", "\(self.tasks.count)")
    ...
    private func updateQueue() {
        ...
        highPriorityTasks.filter({ $0.state == .suspended }).forEach { task in
            clip_log(.debug, "Resume download %@", task.url.absoluteString)
            task.resume()
        }
        ...
        if highQueueHasRunningTasks {
            lowPriorityTasks.filter({ $0.state == .running }).forEach { task in
                clip_log(.debug, "Suspend download %@", task.url.absoluteString)
                task.suspend()
            }
            return
        }
